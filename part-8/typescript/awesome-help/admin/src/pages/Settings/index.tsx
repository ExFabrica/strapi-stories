...
const Settings = () => {
  ...
  const [settings, setSettings] = useState<ISettingsEntity>(undefined);
  const toggleNotification = useNotification();

  //mount
  useEffect(() => {
    SettingsProxy.get().then((data: ISettingsEntity) => {
      setSettings(data);
      setIsLoading(false);
    });
    // unmount
  ...
  const handleSubmit = async () => {
    setIsSaving(true);
    const data:ISettingsEntity = await SettingsProxy.set(settings);
    setSettings(data);
    setIsSaving(false);
    toggleNotification({
      type: 'success',
