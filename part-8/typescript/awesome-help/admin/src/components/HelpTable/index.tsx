...
export const HelpTable = ({ data }) => {
  ...
  const [selectedRow, setSelectedRow] = useState<IHelpEntity>();
  const toggleNotification = useNotification();

  const handleToggle = (): void => {
    setIsModalOpen(prev => !prev);
  };
  const refreshDataGrid = (updatedEntity: IHelpEntity): void => {
  ...
  const handleSave = async (updatedEntity: IHelpEntity): Promise<void> => {
    if (updatedEntity) {
