import IHelpEntity from "../interfaces/help.interface";
...
export default ({ strapi }) => {
  ...
  const findMany = async (params, populate): Promise<IHelpEntity[]> => {
    let helpEntities: IHelpEntity[] = undefined;
    try {
  ...
  const findOne = async (params, populate): Promise<IHelpEntity> => {
    try {
  ...
  const count = async (params): Promise<number> => {
    try {
